...
.topbar-logo {
  ...
  background: var(--s2); border: 1px solid var(--brd); border-radius: var(--r-sm);
...
.topbar-search { flex: 0 1 400px; position: relative; }
.topbar-search svg { position: absolute; left: 12px; top: 50%; transform: translateY(-50%); width: 16px; height: 16px; color: var(--muted); pointer-events: none; }
.topbar-search input {
  width: 100%; height: 38px;
  background: var(--s2); border: 1px solid var(--brd); border-radius: var(--r-full);
  color: var(--tx); font-size: 13px; font-weight: 500; padding: 0 14px 0 36px;
...
.promo-bar button { background: rgba(0,0,0,.2); color: #fff; border-radius: var(--r-full); width: 22px; height: 22px; display: flex; align-items: center; justify-content: center; font-size: 12px; flex-shrink: 0; }

/* ══ PAGE LAYOUT ══ */
.page { display: flex; padding-top: var(--hd-h); min-height: 100dvh; }

.cat-sidebar {
  width: var(--side-w); flex-shrink: 0;
  position: sticky; top: var(--hd-h);
  height: calc(100dvh - var(--hd-h)); overflow-y: auto;
  padding: 24px 0; border-right: 1px solid var(--brd);
...
.content { flex: 1; min-width: 0; padding: 32px 28px 120px; }
...
.cart-sidebar {
  ...
  padding: 20px 20px 32px; display: none;
...
.mobile-nav {
  ...
  border-bottom: 1px solid var(--brd); padding: 12px 0 0;
}
.mobile-search-wrap { padding: 0 16px 10px; }
.mobile-search { position: relative; }
.mobile-search svg { position: absolute; left: 12px; top: 50%; transform: translateY(-50%); width: 17px; height: 17px; color: var(--muted); pointer-events: none; }
.mobile-search input {
  width: 100%; height: 42px;
  background: var(--s2); border: 1px solid var(--brd); border-radius: var(--r-full);
  color: var(--tx); font-size: 14px; font-weight: 500; padding: 0 14px 0 38px;
  transition: border-color .2s;
}
.mobile-search input::placeholder { color: var(--muted); }
.mobile-search input:focus { border-color: var(--p); }
.mobile-cats { display: flex; gap: 8px; overflow-x: auto; padding: 0 16px 12px; scrollbar-width: none; }
...
.mobile-cat-btn {
  ...
  font-size: 12px; font-weight: 700; border: 1px solid var(--brd);
  transition: all .18s; white-space: nowrap;
}
.mobile-cat-btn.ativo { background: var(--p); color: #fff; border-color: var(--p); }

/* ══ SECTION TITLES ══ */
.section-title { font-size: 18px; font-weight: 800; color: var(--tx); margin-bottom: 16px; display: flex; align-items: center; gap: 8px; }
.cat-heading { font-size: 20px; font-weight: 800; color: var(--tx); margin-bottom: 16px; scroll-margin-top: 90px; display: flex; align-items: center; gap: 8px; border-bottom: 1px solid var(--brd); padding-bottom: 12px; }
.cat-section { margin-bottom: 40px; scroll-margin-top: 90px; }

/* ══ FEATURED CARDS ══ */
.featured-section { margin-bottom: 40px; }
.featured-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(160px, 1fr)); gap: 14px; }
.featured-card {
  background: var(--s1); border: 1px solid var(--brd); border-radius: var(--r-lg);
...
.featured-img { width: 100%; aspect-ratio: 1; background: var(--s2); border-radius: var(--r); display: flex; align-items: center; justify-content: center; font-size: 56px; overflow: hidden; }
...
.prod-card {
  ...
  display: flex; cursor: pointer; transition: all .15s; overflow: hidden;
}
...
.prod-badge { display: inline-flex; align-items: center; gap: 3px; padding: 2px 8px; border-radius: var(--r-full); font-size: 10px; font-weight: 700; text-transform: uppercase; letter-spacing: .03em; }
...
.prod-desc { font-size: 12px; color: var(--muted); line-height: 1.5; margin-top: 4px; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden; }
...
.btn-add { flex-shrink: 0; background: var(--p); color: #fff; border-radius: var(--r-full); padding: 7px 16px; font-size: 13px; font-weight: 700; transition: background .15s, transform .1s; white-space: nowrap; }
.btn-add:hover  { background: var(--pd); }
.btn-add:active { transform: scale(.93); }
.prod-img { width: 120px; flex-shrink: 0; background: var(--s2); display: flex; align-items: center; justify-content: center; font-size: 52px; overflow: hidden; }
...
.prod-grid .prod-img { width: 100%; height: 160px; border-radius: 0; }
...
.modal-img {
  ...
  background: var(--s2);
  display: flex; align-items: center; justify-content: center;
  font-size: 88px; overflow: hidden; flex-shrink: 0;
}
.modal-img img { width: 100%; height: 100%; object-fit: cover; }

.modal-content { padding: 20px 22px 0; }

.modal-nome { font-size: 22px; font-weight: 800; margin-bottom: 6px; line-height: 1.3; }
.modal-desc { font-size: 14px; color: var(--muted); line-height: 1.6; margin-bottom: 14px; }
.modal-preco-area { margin-bottom: 22px; }
.modal-preco-old { font-size: 12px; color: var(--muted); text-decoration: line-through; margin-bottom: 2px; }
.modal-preco { font-size: 26px; font-weight: 900; color: var(--p); }

/* ── Opcoes section header ── */
.opcoes-section { margin-bottom: 4px; }
.opcoes-header {
...
.opcoes-header-left {}
.opcoes-header-title { font-size: 14px; font-weight: 800; }
.opcoes-header-sub { font-size: 12px; color: var(--muted); margin-top: 2px; }
.opcoes-check {
...
/* ── Opcoes list ── */
.opcoes-list {
  border: 1px solid var(--brd); border-radius: 0 0 var(--r) var(--r);
...
.opcao-item:hover { background: var(--s3); }
.opcao-item.selecionado { background: rgba(232,93,4,.06); }
...
.opcao-emoji img { width: 100%; height: 100%; object-fit: cover; }
.opcao-label { flex: 1; min-width: 0; }
.opcao-nome { font-size: 14px; font-weight: 600; }
.opcao-preco { font-size: 12px; color: var(--p); font-weight: 700; margin-top: 2px; }

/* Toggle remover = checkbox style */
.opcao-toggle {
  width: 28px; height: 28px; flex-shrink: 0;
  border-radius: var(--r-full); border: 2px solid var(--brd);
  display: flex; align-items: center; justify-content: center;
...
.opcao-btn-extra {
  width: 28px; height: 28px; flex-shrink: 0;
  border-radius: var(--r-full); border: 1.5px solid var(--brd);
  display: flex; align-items: center; justify-content: center;
...
/* ── Comentário ── */
.obs-wrap { margin-bottom: 22px; }
.obs-header { display: flex; align-items: center; justify-content: space-between; margin-bottom: 8px; }
.obs-label { font-size: 13px; font-weight: 700; color: var(--tx); }
.obs-counter { font-size: 11px; color: var(--muted); font-weight: 600; font-variant-numeric: tabular-nums; }
.obs-input {
  width: 100%; background: var(--s2); border: 1px solid var(--brd); border-radius: var(--r);
  color: var(--tx); font-size: 14px; padding: 12px 14px; resize: none; height: 82px;
  transition: border-color .2s;
}
...
@supports (padding-bottom: env(safe-area-inset-bottom)) {
  .modal-footer-fixed { padding-bottom: max(20px, calc(env(safe-area-inset-bottom) + 14px)); }
}
.qtd-ctrl { display: flex; align-items: center; background: var(--s2); border: 1px solid var(--brd); border-radius: var(--r); overflow: hidden; }
.qtd-btn { width: 42px; height: 50px; display: flex; align-items: center; justify-content: center; color: var(--tx); font-size: 20px; font-weight: 700; transition: background .15s; }
...
.cart-item { background: var(--s2); border: 1px solid var(--brd); border-radius: var(--r); padding: 11px 12px; display: flex; gap: 10px; align-items: flex-start; }
.ci-emoji { font-size: 28px; flex-shrink: 0; }
.ci-body { flex: 1; min-width: 0; }
.ci-name { font-size: 13px; font-weight: 700; }
.ci-extras { font-size: 11px; color: var(--p); margin-top: 2px; }
...
.cupom-input { flex: 1; height: 40px; background: var(--s2); border: 1px solid var(--brd); border-radius: var(--r-sm); color: var(--tx); font-size: 12px; padding: 0 12px; text-transform: uppercase; letter-spacing: .04em; transition: border-color .2s; }
.cupom-input:focus { border-color: var(--p); }
.btn-cupom { height: 40px; padding: 0 14px; background: var(--s2); border: 1px solid var(--brd); border-radius: var(--r-sm); color: var(--tx); font-size: 12px; font-weight: 700; transition: border-color .15s; }
...
.btn-finalizar {
  ...
  display: flex; align-items: center; justify-content: center; gap: 8px;
  transition: background .15s; box-shadow: 0 4px 16px rgba(34,197,94,.3); margin-top: 12px;
}
...
.drawer-head { padding: 16px 20px 12px; border-bottom: 1px solid var(--brd); display: flex; align-items: center; justify-content: space-between; flex-shrink: 0; }
...
@media (min-width: 768px) {
  ...
  .mobile-nav { top: var(--hd-h); display: none; }
  ...
  .cat-section, .cat-heading { scroll-margin-top: 80px; }

  /* Modal desktop: imagem esquerda, conteúdo direita */
  .modal { max-width: 780px; max-height: 88dvh; border-radius: 20px; bottom: auto; top: 50%; left: 50%; transform: translate(-50%, calc(-50% + 30px)); transition: transform .3s cubic-bezier(.4,0,.2,1), opacity .3s; opacity: 0; pointer-events: none; }
  .modal.open { transform: translate(-50%, -50%); opacity: 1; pointer-events: all; }
  .modal-layout { flex-direction: row; padding: 0; align-items: stretch; }
  .modal-img { width: 42%; aspect-ratio: auto; border-radius: 0; min-height: 340px; }
  .modal-left { width: 42%; flex-shrink: 0; }
  .modal-right { flex: 1; overflow-y: auto; padding: 0; display: flex; flex-direction: column; scrollbar-width: thin; }
  .modal-content { padding: 24px 24px 0; }
  .modal-footer-fixed { padding: 14px 24px 20px; border-radius: 0 0 20px 0; }
  .modal-handle { display: none; }
  .modal-x { position: absolute; top: 14px; right: 16px; float: none; margin: 0; }
}
...
.content-search input { width: 100%; height: 46px; background: var(--s2); border: 1px solid var(--brd); border-radius: var(--r-full); color: var(--tx); font-size: 14px; font-weight: 500; padding: 0 16px 0 44px; transition: border-color .2s; }
    ...
    <div class="modal-right" id="modalRight">
      <div class="modal-content">
        ...
        <div class="obs-wrap">
          ...
          <textarea class="obs-input" id="modalObs" maxlength="140" placeholder="Ex: tirar a cebola, maionese à parte etc." oninput="updCounter()"></textarea>
        </div>
      </div>

      <!-- Footer fixo dentro do modal (dentro da coluna direita no desktop) -->
...
function abrirModal(prod) {
  ...
  // scroll modal to top
  document.getElementById('modalProduto').scrollTop = 0;
}
...
<?php
/**
 * Helper: monta o array de dados do produto incluindo opções (remover/extras)
 */
function buildProdData(array $p, array $opcoesPorProduto): array {
    $id = (int)$p['id'];
    $remover = [];
    $extras  = [];
    if (isset($opcoesPorProduto[$id])) {
        foreach ($opcoesPorProduto[$id]['remover'] ?? [] as $o) {
            $remover[] = [
                'id'    => (int)$o['id'],
                'nome'  => $o['nome'],
                'emoji' => $o['emoji'] ?? '🚫',
        ...
        foreach ($opcoesPorProduto[$id]['extra'] ?? [] as $o) {
            $extras[] = [
                'id'    => (int)$o['id'],
                'nome'  => $o['nome'],
                'preco' => (float)$o['preco'],
