...
function saveCart(c) { localStorage.setItem('carrinho', JSON.stringify(c)); }
...
function addToCart(produto_id, nome, preco, quantidade, obs, variacoes) {
  const c = getCart();
  const key = produto_id + '_' + (variacoes||[]).join('_');
  const idx = c.findIndex(i => i._key === key);
  if (idx >= 0) c[idx].quantidade += quantidade;
  else c.push({ _key: key, produto_id, nome, preco, quantidade, obs: obs||'', variacoes: variacoes||[] });
  saveCart(c);
  updateCartBtn();
}
...
function abrirProduto(id, nome, preco, desc, img) {
  ...
  document.getElementById('variacoesWrap').innerHTML = '<p style="color:var(--muted);font-size:13px;text-align:center;padding:8px 0">Carregando opções...</p>';
  atuPreco();
  ...
  fetch('api/variacoes_produto.php?produto_id=' + id)
    .then(r => r.json()).then(grupos => {
      const wrap = document.getElementById('variacoesWrap');
      const keys = Object.keys(grupos);
      if (!keys.length) { wrap.innerHTML = ''; return; }
      wrap.innerHTML = keys.map(g => `
        <div style="margin-bottom:14px">
          <div style="font-weight:700;font-size:13px;color:var(--muted);text-transform:uppercase;letter-spacing:.5px;margin-bottom:6px">${g}</div>
          ${grupos[g].map(v => `
            <label style="display:flex;align-items:center;gap:10px;padding:10px;border:2px solid var(--border);border-radius:10px;cursor:pointer;margin-bottom:6px;font-size:14px;transition:border-color .15s">
              <input type="radio" name="v_${g.replace(/\s/g,'_')}" value="${v.id}" data-extra="${v.preco_extra}" style="accent-color:var(--p);width:18px;height:18px">
              <span style="flex:1">${v.nome}</span>
              ${v.preco_extra != 0 ? `<span style="color:var(--p);font-weight:700">${v.preco_extra > 0 ? '+' : ''}${fmt(v.preco_extra)}</span>` : ''}
            </label>
          `).join('')}
        </div>`).join('');
      wrap.querySelectorAll('input[type=radio]').forEach(inp => {
        inp.addEventListener('change', function() {
          ...
          this.closest('label').style.borderColor = 'var(--p)';
        });
      });
    }).catch(() => { document.getElementById('variacoesWrap').innerHTML = ''; });
}
...
function atuPreco() {
  const extra = Object.values(_mvar).reduce((s,v) => s + v.extra, 0);
  const unit = _mpreco + extra;
  document.getElementById('modalPreco').textContent = fmt(unit);
  document.getElementById('modalBtnPreco').textContent = fmt(unit * _mqtd);
}

function adjQtd(d) {
  _mqtd = Math.max(1, _mqtd + d);
  document.getElementById('modalQtd').textContent = _mqtd;
  atuPreco();
}
...
function confirmarModal() {
  ...
  addToCart(_mid, nome, preco, _mqtd, obs, varIds);
  fecharModal();
